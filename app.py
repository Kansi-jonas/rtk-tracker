...
PHASE_ID = "UC_MID1CI"
CREATED_FILE = "created_leads.txt"
...
def track_click(lead_id):
    # 🔒 Blockiere HEAD-Anfragen (z. B. von Bots, Previews)
    if request.method == "HEAD":
        print(f"⚠️ HEAD request for {lead_id} ignored.")
    ...
    if os.path.exists(CREATED_FILE):
        with open(CREATED_FILE, "r") as f:
            if lead_id in f.read().splitlines():
                print(f"⚠️ Lead {lead_id} already created, skipping.")
                return redirect(REDIRECT_URL)
